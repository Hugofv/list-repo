...
describe('List tests', () => {
  const mockRepos = [
    {
      ...
      git_tags_url:
        'https://api.github.com/repos/test/HelloWorld/git/tags{/sha}',
      git_refs_url:
        'https://api.github.com/repos/test/HelloWorld/git/refs{/sha}',
      trees_url: 'https://api.github.com/repos/test/HelloWorld/git/trees{/sha}',
      statuses_url:
        'https://api.github.com/repos/test/HelloWorld/statuses/{sha}',
      ...
      subscribers_url:
        'https://api.github.com/repos/test/HelloWorld/subscribers',
      subscription_url:
      ...
      issues_url:
        'https://api.github.com/repos/test/HelloWorld/issues{/number}',
      ...
      releases_url:
        'https://api.github.com/repos/test/HelloWorld/releases{/id}',
      deployments_url:
        'https://api.github.com/repos/test/HelloWorld/deployments',
  ...
  test('should render Detail as loading state', () => {
    ...
    expect(screen.getByTestId(/list-loading/i)).toBeInTheDocument();
  });

  test('should render Detail as show list', () => {
    const store = mockStore({
      repo: {
        loading: false,
        error: false,
  ...
  test('should render Detail as not found', () => {
    ...
    render(
      <Provider store={store}>
        <List repos={[]} selectedRepo={mockRepos[0]} handleRepo={handleRepo} />
      </Provider>
    );
  ...
  test('should render Detail change repo', () => {
    const store = mockStore({
      repo: {
        loading: false,
        error: false,
    ...
    fireEvent.click(screen.getByTestId(/repo-0/i));
    expect(handleRepo).toBeCalled();
  });
});
